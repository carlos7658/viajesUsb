...
public class Utilities {
    ...
    //Retorna si un string es vacio o no 
	...
	public static boolean isOnlyletras(String param) {
		param=param.replace(" ", "");
		if(param.matches("[a-zA-Z]"+"{"+param.length()+"}")) {
			return true;
		}
		return false;
	}
	...
	//
	public static boolean isGreater(String text,int number) {
		if(text!=null) {
	...
	public static boolean isOnlyletras2(String param) {
		if(param.matches("^\\w+(\\s\\w+)*$"+"{"+param.length()+"}")) {
			return true;
		}
		return false;
	}
